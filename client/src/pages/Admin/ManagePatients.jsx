          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {patients.map((pat) => (
                  <tr key={pat._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10">
                    <td className="py-4 px-4 font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                      <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400">
                        <User size={14} />
                      </div>
                      <span>{pat.userId?.name}</span>
                    </td>
                    <td className="py-4 px-4 text-slate-500 dark:text-slate-400">{pat.userId?.email}</td>
                    <td className="py-4 px-4 font-semibold text-slate-700 dark:text-slate-300">{pat.phone || 'N/A'}</td>
